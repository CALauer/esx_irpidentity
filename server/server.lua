...
--[[TriggerEvent('es:addCommand', 'testing2',  function(source)	
		local xPlayer = ESX.GetPlayerFromId(source)	 -------/HAVE TO HAVE THESE          -----------------XPLAYER @ IDENTIFIER
		local identifier = GetPlayerIdentifiers(source)[1]	-------/HAVE TO HAVE THESE
		MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `identifier` = @identifier', {
			['@identifier'] = identifier,
		}, function(result)
			if result[1].firstname ~= nil then
...
end)]]-- 100% Working ConfigurationRegisterServerEvent("setJob")
...
TriggerEvent('es:addGroupCommand', 'charlist', 'user', function(source, args, user)
	TriggerEvent('getCharacters', source, function(data)
		if data.firstname1 ~= '' then
			...
			TriggerClientEvent('chat:addMessage', source, { args = { '^[ImpulseRP]', 'You have no registered characters. Use the ^3/register^0 command to register a character.' } })
		end
	end)
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^3SYSTEM', 'Insufficient permissions!' } })
